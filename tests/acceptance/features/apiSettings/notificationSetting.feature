...
Feature: Notification Settings
  ...
  Scenario: disable mail notification for Space deleted event
    Given the administrator has assigned the role "Space Admin" to user "Alice" using the Graph API
    And user "Alice" has created a space "new-space" with the default quota using the Graph API
    And user "Alice" has sent the following space share invitation:
      | space           | new-space    |
            ...
            "properties": {
              "identifier":{
                ...
                "properties": {
                  ...
                  "setting":{
                    "const": "event-space-deleted-options"
                  }
                }
              },
              "value":{
    ...
    And user "Brian" should get a notification with subject "Space shared" and message:
      | message                                   |
      | Alice Hansen has disabled Space new-space |
    But user "Brian" should not have a notification related to space "new-space" with subject "Space deleted"
